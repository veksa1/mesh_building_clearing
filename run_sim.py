...
def main() -> int:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  --comm-backend inproc (default): RadioMedium delivers packets inside one Python process.\n"
            "  --comm-backend udp — same propagation model, one OS process per drone; packets go over\n"
            "                     real UDP localhost with RSSI-style decode (--udp-base-port selects base listen port).\n\n"
            "Examples:\n"
    ...
    parser.add_argument(
        ...
        "`inproc` single-process RadioMedium (default); "
        "`udp` multiprocess takeover sim via PropagationUDPTransmission (listen port = udp-base-port + uid).",
    )
    parser.add_argument(
        "--udp-base-port",
        type=int,
