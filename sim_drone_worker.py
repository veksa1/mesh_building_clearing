...
def sim_drone_worker_main(
    ...
    *,
    ...
    building_blob: bytes | None = None,
) -> None:
    """Worker loop driven by tuples on ``inbound``; replies on ``outbound``.

    ``building_blob`` (pickled :class:`BuildingMap`) is used when the kernel has a
    rasterised floorplan that isn't a registered builder layout (e.g. live exports
    from the web). When omitted, the worker falls back to ``load_layout(layout_name)``.
    """

    rng_state: bytes | None = None
    if building_blob is not None:
        building_map = pickle.loads(building_blob)
    else:
        building_map = load_layout(layout_name)
    ...
    try:
        while not stop_event.is_set():
            ...
            if tag == "INIT":
                ...
                assert agent is not None
                # Trail the actual belief portal-edges so the kernel can serialise the explorer's set
                # into the web bundle (BeliefTreePanel input).
                outbound.put(
                    (
                        "STATS_LOCAL",
                        int(uid),
                        ...
                        len(agent.recent_detections),
                        dict(agent.neighbors_last),
                        tuple(sorted(agent.belief_edges)),
                    )
                )
    finally:
        try:
